class AtmosphereClient {
    ...
    RenderDnsRecordsReport(ResultData, ContentElement) {
        const BuildBadgeList = (Entries, ColorName) => (Entries && Entries.length > 0 ? Entries.map((Entry) => `<span class="Badge Badge-${ColorName}">${Entry}</span>`).join(" ") : "None Found");
    ...
    RenderSslCheckReport(ResultData, ContentElement) {
        ...
        const ExpiryBadge = Certificate.IsExpired ? this.WrapBadge("Expired", "Red") : Certificate.DaysUntilExpiry <= 14 ? this.WrapBadge(`${Certificate.DaysUntilExpiry} Days Left`, "Orange") : this.WrapBadge(`${Certificate.DaysUntilExpiry} Days Left`, "Green");

        const ChainRowsHtml = ResultData.ChainInfo.map(
            (ChainCertificate, ChainIndex) => `
    ...
    async RunPortCheck(TargetHost) {
        ...
        try {
            ...
            const PortRowsHtml = ResultData.Ports.map(
                (PortEntry) => `
        <tr>
    ...
    async RunPing(TargetHost) {
        ...
        try {
            ...
            const AttemptRowsHtml = ResultData.Attempts.map(
                (Attempt) => `
        <tr>
    ...
    async RunBlacklistCheck(TargetIp) {
        ...
        try {
            ...
            const ZoneRowsHtml = (ResultData.Entries || [])
                .map(
                    (Entry) => `
        <tr>
      ...
      `
            ...
            const OverallBadge = ResultData.ListedCount > 0 ? this.WrapBadge(`Listed On ${ResultData.ListedCount} Of ${ResultData.TotalChecked}`, "Red") : this.WrapBadge(`Clean On All ${ResultData.TotalChecked} Zones`, "Green");
    ...
    async RunPreview(TargetUrl) {
        ...
        try {
            ...
            const PreviewImageHtml = ResultData.PreviewImageUrl ? `<img src="${ResultData.PreviewImageUrl}" alt="Preview" class="PreviewImage">` : `<div class="PreviewImagePlaceholder"><i class="fa-solid fa-image"></i> No Preview Image Found</div>`;
    ...
    async RunRedirectTrace(TargetUrl) {
        ...
        try {
            ...
            const HopRowsHtml = (ResultData.Hops || [])
                .map(
                    (Hop) => `
        <tr>
    ...
    async InitializeGlobe(GlobeContainerElement) {
        ...
        const SphereGeometry = new window.THREE.SphereGeometry(1, 32, 32);
        ...
        const VisibilityHandler = () => {
            ...
            if (IsPageVisible && !AnimationFrameHandle) {
                AnimateFrame();
            } else if (!IsPageVisible && AnimationFrameHandle) {
    ...
    StopAllActiveGlobes() {
        this.ActiveGlobeInstances.forEach((GlobeInstance) => {
            GlobeInstance.StopAnimation();
            GlobeInstance.ContainerElement.dataset.initialized = "false";
        });
        this.ActiveGlobeInstances = [];
    }
    ...
    async BuildContinentOutlines() {
        ...
        LandFeature.features.forEach((LandFeatureEntry) => {
            ...
            const PolygonList = GeometryType === "Polygon" ? [LandFeatureEntry.geometry.coordinates] : LandFeatureEntry.geometry.coordinates;
    ...
    RenderBatchReport(BatchResult, ContentElement) {
        const ResultItemsHtml = (BatchResult.Results || [])
            .map((GeoData) => {
                const LocationLine = [GeoData.City, GeoData.Region, GeoData.Country].filter(Boolean).join(", ") || "Location Unresolved";
                return `
      ...
      `;
        ...
        const FailedHtml = BatchResult.Failed && BatchResult.Failed.length > 0 ? `<div class="BatchFailedList">Failed to resolve: ${BatchResult.Failed.join(", ")}</div>` : "";
